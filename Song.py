	...
	def io(self, filename, sampling_freq, is_target):
		...
		if cur_sampling_freq != self.sampling_freq:
			...
			if isinstance(self.data[0], (list, tuple, np.ndarray)):
				data_mono = np.mean(self.data, axis=1)
			else:
				data_mono = self.data
			# compute new number of samples and resample using scipy.signal.resample
			new_len = int(len(data_mono) * float(self.sampling_freq) / float(cur_sampling_freq))
			self.data = signal.resample(data_mono, new_len)
			cur_sampling_freq = self.sampling_freq
...
def fft(x, window_func, window_size):
	...
	l = len(x)
	# Add padding in case length of data is not the window size
	if l != window_size:
	...
	# Compute FFT using numpy (fast)
	# rfft returns window_size//2 + 1 bins (including Nyquist); original code used window_size//2 bins
	spectrum = np.fft.rfft(x)
	spectrum = spectrum[:-1]  # drop Nyquist to match original length
	mods = np.abs(spectrum)
	# avoid log10(0)
	mods = np.where(mods <= 0, 1e-10, mods)
