...
#Dump spectogram pickle dump for each song in the folder self.data_path
	def make_spectogram_database(self):
		...
		for song_name in os.listdir(self.audio_path):
			...
			if not song_name.lower().endswith('.wav'):
				continue
...
#Hash all the songs in the audio/ folder and store it as a pkl file
	def hash_database(self):
		...
		for songname in sorted(os.listdir(self.audio_path)):
			# only hash wav files
			if not songname.lower().endswith('.wav'):
				continue
...
#Return only those couples in the database whose song_id matches the names in the list of desired song_names
	...
	def compare_song(self, song):
		...
		# Simple song-hit counts across all matching keys (fallback)
		# key: song_name, value: count of occurrences across matching keys
		song_hits = {}
		for key in cur_song_hash.keys():
			if key in self.database:
				for tval in self.database[key]:
					sname = tval[1]
					song_hits[sname] = song_hits.get(sname, 0) + 1

		# Compute top fallback suggestion (based on raw hit counts)
		if song_hits:
			sorted_hits = sorted(song_hits.items(), key=lambda x: x[1], reverse=True)
			self.last_fallback = (sorted_hits[0][0], sorted_hits[0][1])
			# also prepare a short printed list
			self._fallback_list = sorted_hits[:10]
		else:
			self.last_fallback = (None, 0)
			self._fallback_list = []
		...
		# If no coherent notes were found, try a simple frequency-based fallback using song-hit counts
		if not coherent_notes:
			print("No coherent matches found.")
			if self._fallback_list:
				print("Fallback suggestions (song, hit_count):")
				for s, cnt in self._fallback_list:
					print(s, cnt)
				# return the top suggestion as a weak match
				best_song = self._fallback_list[0][0]
				return (best_song, None)
			else:
				print('No fallback hits either.\n')
				return (None, None)

		# Also always print the top fallback suggestion for visibility
		if getattr(self, '_fallback_list', None):
			fb = self._fallback_list[0]
			print("\nFallback top suggestion:", fb[0], "(hit_count=", fb[1], ")")
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Identify a short audio clip against the audio database.')
	parser.add_argument('target', nargs='?', default='test/A_Sky_Full_of_Stars_12_3.wav', help='Path to target WAV file (default: test/A_Sky_Full_of_Stars_12_3.wav)')
	parser.add_argument('--audio-dir', default='audio/', help='Directory containing database audio WAVs (default: audio/)')
	parser.add_argument('--data-dir', default='data/', help='Directory to store spectrogram pickles and database (default: data/)')
	parser.add_argument('--rebuild', action='store_true', help='Force rebuild of the fingerprint database')
	args = parser.parse_args()

	# make Findit object
	app = Findit(audio_path=args.audio_dir, data_path=args.data_dir)

	# optionally force rebuild
	if args.rebuild:
		dbfile = os.path.join(args.data_dir, 'database.pkl')
		if os.path.exists(dbfile):
			print('Removing existing database pickle to force rebuild:', dbfile)
			os.remove(dbfile)
		app.database = {}
	...
	song = app.create_song(args.target, try_dumped=True, is_target=True)
